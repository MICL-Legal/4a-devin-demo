...
- 2026-08-14 08:45（裝置：MT-N112）：大批更新，準備 commit + push：①「複製圖片」按鈕從單一 `id` 改成 `.imgcopybtn` class + `closest('.imgbox')` 找對應圖片，支援同頁多組圖片複製（不再只認一個 `#imgCopyBtn`）。②例子1：新增「加碼一句」（快遞／海運下拉選單 prompt）＋「💡換成你的」改成問「你要算什麼、數據在哪」＋新增「📎範例檔」摺疊區塊（`assets/001-quote-reference.jpg`＋`.html`，含運送方式版報價系統，取自既有 `project/tutorial/2026_4a/order-demo-5`）。③例子2：why 文字簡化（拿掉 Billy 場／2-4 那段）＋新增「📎範例檔」摺疊區塊，內嵌真的生成出來的示範影片 `assets/002-mpt-sample.mp4`（15MB，可播放/下載）。④例子3：新增「③使用者第二次追加」步驟教下拉選單選項描述法，**公司別故意用 A、B、C、D 佔位**（原始畫面的下拉選項含真實廠商名單，使用者主動示警，已確認未曾寫入任何檔案）；「💡換成你的」改成更通用的說法；派 agent 產出完整可互動的報表整合系統範例（`assets/003-report-reference.html`，agent 自行確認未出現任何真實廠商/銀行名稱），使用者自行截圖回傳（`assets/003-report-reference.jpg`），一併加進「📎範例檔」摺疊區塊。⑤例子4：補上「📎範例檔」摺疊區塊，圖取自使用者截圖的首頁畫面（`assets/004-expo-reference.jpg`），HTML 連結既有的 `card-scanner-app.html`。⑥新增 CSS：`a.dlfile`、`video.vidbox`。Claude in Chrome 擴充功能全程未連線，所有截圖都是使用者手動截圖後貼回來、我再存檔。
